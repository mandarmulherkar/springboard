...
ag.fico.groups = read.csv("91_mp_people.csv", header = TRUE)
...
library(NbClust)
set.seed(1234)
nc <- NbClust(df, min.nc=3, max.nc=7, method="kmeans")
table(nc$Best.n[1,])
...
plot(data[c("income", "sessions")], col = fit.km$cluster)
library(ggplot2)
ggplot(data[c("fico_group", "number_of_trims_selected")], col = fit.km$cluster)